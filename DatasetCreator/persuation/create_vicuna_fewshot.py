from utils import (
    ...
    read_captions_file, create_few_shot_examples,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_in_context_examples', type=int, help='Number of in context examples')
    ...
    num_in_context_examples = args.num_in_context_examples or 3
    ...
    prompt_template = """
        There is a meme with text written inside the meme.
        ...
        Text Writtent in the meme: \"<MemeText>\" \n \
        Caption: \"<MemeCaption>\" \n \
        Examples: \n 
    """
    ...
    for d in tqdm(persuation_dataset):  
        random_indexes = generate_random_numbers(len(persuation_dataset), num_in_context_examples, [i])
        examples = create_few_shot_examples(random_indexes, persuation_dataset, persuasion_captions)
        examples = " \n ".join(examples)
    ...
    with open(os.path.join(vicuna_dataset_path, f'vicuna_dataset_{num_in_context_examples}_shots.jsonl'), 'w') as vicuna_dataset_file:
